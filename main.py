...
def log(phrase, status=None):
    global GLOBAL_VARIABLES
    with GLOBAL_VARIABLES["print"]:
        if status == "Error":
            print(Fore.RED + phrase)
        elif status == "Success":
            print(Fore.GREEN + phrase)
        else:
            print(Fore.RESET + phrase)
    ...
    if is_verified == True:
        with requests.Session() as s:
            ...
            if (KITH_RESP.url == "https://kith.com/challenge") or (KITH_RESP.url == "https://kith.com/challenge/"):  # captcha required
                ...
                log("Successfully logged in with captcha.", "Success")
                unlock_p(rand_proxy)
                return s.cookies
            else:  # no captcha required
                log("Successfully logged in without captcha.", "Success")
                unlock_p(rand_proxy)
                return s.cookies
    else:
        return is_verified
...
def enter_raffle(accs_tuple, url):
    ...
    while GLOBAL_VARIABLES["queue_"].qsize() > 0:
        try:
            log("Accounts left: " + str(GLOBAL_VARIABLES["queue_"].qsize()))
            with GLOBAL_VARIABLES["q_lock_"]:
            ...
            if not rand_proxy in GLOBAL_VARIABLES["p_ll"]:
                    ...
                    if (log_cookies_ != False) and (log_cookies_ != "banned"):
                        ...
                        if (not thread_driver.checkentry()):
                            ...
                            if thread_driver.checkentry():
                                log("Successfully entered raffle.", "Success")
                                thread_driver.refresh()
                ...
                log("Selected proxy is in use.", "Error")
                with GLOBAL_VARIABLES["q_lock_"]:
                    GLOBAL_VARIABLES["queue_"].put(1)
        except Exception:
...
def request_recaptcha(session):
    ...
    if resp.text[0:2] != 'OK':
        log("Error: {} Exiting...".format(resp.text), "Error")
    captcha_id = resp.text[3:]
    log("Successfully requested for captcha.", "Success")
    return captcha_id
...
def receive_token(captcha_id, session):
    ...
    for count in range(1, 26):
        ...
        if resp.text[0:2] == 'OK':
            ...
            log("Captcha token received.", "Success")
            return grt
        sleep(5)
    log("No tokens received. Restarting...", "Error")
...
def wrapper_(accs_tuple, url):
    ...
    threads = []
    for _ in range(10):
        t = Thread(target=enter_raffle, args=(accs_tuple, url))
        threads.append(t)
    [t.start() for t in threads]
    [t.join() for t in threads]
    if str(platform.system()) == "Windows":
        log("Cleaning up any left over drivers...")
        os.chdir("classes")
        os.system("killcdriver.bat")
    log("Finished in " + str(time() - st), "Success")
